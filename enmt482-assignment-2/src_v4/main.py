...
#
#  Module Description:
#  ENMT482 Robotics Project II - RoboDK Coffee-Making Routine
# 
#           Made with sweat, tears and Python 3.6 
# 
...
class Grinder:
    ...
    P_G_PFdropoff_OFFSET_Z = 0.5
    P_G_PFdropoff = np_col(157.61 + 5, 0+1, -250.45 + P_G_PFdropoff_OFFSET_Z)  # +5 fudge factor added by @Sam after watching Sim
    ...
    P_leverDepressed_OFFSET_Z = 2 
    P_G_leverDepressed = np_col(-35.82, 83.80, -153.00+25+P_leverDepressed_OFFSET_Z)
    ...
    def routine():
        ...
        # # A.1 Get filter tool
        Grinder.get_filter_from_global_home()
    ...
    @staticmethod
    def get_filter_from_global_home():
        waypoint_to_toolstand_1 = [-67.440000, -77.380000, -84.030000, -128.480000, 13.750000, -28.010000]
        waypoint_to_toolstand_2 = [-106.860000, -77.380000, -84.030000, -128.480000, 13.750000, -28.010000]
    ...
    def pull_doser():
        ...
        for _ in range(3):
            ...
            rdk.pause(0.5)
            # Add moveJ due to bug with RoboDK software causing crash after circular moves
            robot.MoveJ(lever_start_pos, blocking=True)
...
class Tamper:
    ...
    tamperInside = 10.5 # how much to push the tamper up when tamping  (ie how far the tamper goes inside the portafilter)
...
class ToolStand:
    ...
    # Group head
    GH_INSERSION_RAISE = 10 # 30 #21.6988 #mm # Z = 188.6988 is about where it crosses threshold 
    GH_OFFSET_X = 0
    GH_OFFSET_Y = -1
    ...
    def insert_portafilter_in_group_head_and_release():
        ...
        # Add moveJ due to bug with RoboDK software causing crash after circular moves
        robot.MoveJ(get_T2RDK(T_B_M__groupHeadInserted), blocking=True) 
...
class EspressoMachine:
    ...
    T_E_btn1 = get_T_given_P_and_R(P_E_btn1, get_R_given_theta(0, 'y'))
...
def main():
    ...
    # Starting from global home run through grinder routine
    Grinder.routine()
