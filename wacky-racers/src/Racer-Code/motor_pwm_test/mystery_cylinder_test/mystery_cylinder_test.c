...
int main (void)
{
	...
	pio_config_set (SERVO_PWM_PIO, PIO_OUTPUT_HIGH);
    ...
    if ((inPwmVal != oldPwmVal) || (motor_select != old_motor_select)){
        ...
        if (inPwmVal >= 50){
            ...
            pio_config_set (SERVO_PWM_PIO, PIO_OUTPUT_LOW);
            pwm_duty_ppt_set (servo_pwm, (pwm_val_servo_adjusted)*20);
        }
    }
