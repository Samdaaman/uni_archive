...
K_start = [13.4085,-1.2371,-1.1714,3.801,0.9853,0.2774]; % gains from LQR

grid_widths = abs(K_start) / 4; % tuning parameter
K_guesses = [K_start];
lowest_gammas = [NaN];

for ii = 1:50 % do 50 iterations (tuning paramter)
%     % HOT H-inf spice
%     K_guess_min = K_guesses(ii, :) - grid_widths / 2;
%     K_guess_max = K_guesses(ii, :) + grid_widths / 2;

    % Medium-rare H-inf spice
    K_guess_min = max(K_guesses(ii, :) - grid_widths / 2, K_start - abs(K_start) * 2 / 3);
    K_guess_max = min(K_guesses(ii, :) + grid_widths / 2, K_start + abs(K_start) * 2 / 2);
...
%     K_guess_max = K_guesses(ii, :) + grid_widths / 2;
    ...
    % Do a grid search using the gains for the current iteration and try to
    % find better gains (ones with a lower gamma)
    ...
    grid_widths = grid_widths * 0.95; % reduce the grid size  (recursive)
end
...
% (uses LMI inequality with Shur's complement form)
% A core function used in Hinf controller design functions
% which iteratively determine the lowest gamma
...
% Iteratively find an approximate minimum feasible gamma by searching
% downwards from a start point
% Used to "score" a given A_CL to find a minimum gamma (disturance rejection)
...
% This allows for optimising a given set of gains with H-inf control
