...
K_start = [13.4085,-1.2371,-1.1714,3.801,0.9853,0.2774];
...
%% ZP Added: Pole determination
poles_K_guesses = [];
for jj=1:length(K_guesses)
    A_CL = (A - B1*K_guesses(jj,:))
    N = -(C * A_CL^-1 * B1)^-1; % reference tracking
    B_hat = B1*N*step_size; % input matrix adjusted for step size with reference tracking
    curr_ss = ss(A_CL, B_hat, C, 0);
    poles_K_guesses(jj,:) = pole(curr_ss);
end
...
prev_poles = poles_K_guesses(1,:);
for ii = 1:length(poles_K_guesses)
    curr_poles = poles_K_guesses(ii,:);
    for jj = 1:length(curr_poles)
        curr_pole = curr_poles(jj);
        real_component = [real(prev_poles(jj)), real(curr_pole)];
        imag_component = [imag(prev_poles(jj)), imag(curr_pole)];
        
        if (ii < 2)
            marker_colour = '#ff0000';
            plot(real(curr_pole), imag(curr_pole), 'x', 'Linewidth', 1, 'Markersize', 5, 'color', marker_colour);
        elseif (ii == length(poles_K_guesses))
            marker_colour = '#00ff00';
            plot(real(curr_pole), imag(curr_pole), 'x', 'Linewidth', 1, 'Markersize', 5, 'color', marker_colour);
        else
            marker_colour = mark_colours(jj);
            plot(real_component, imag_component, '-', 'Linewidth', 1, 'Markersize', 5, 'color', marker_colour);
        end
        
        %plot(real(curr_pole), imag(curr_pole), 'x', 'Markersize', 8,
        %'Linewidth', 1, 'color', marker_colour);
    end
    prev_poles = curr_poles;
end
hold off
legend("Red: K start", "Green: K end")
%%
...
hold on
yyaxis right
% plots relative wondering of each gain
% each iteration plots different gain
for ii = 1:6
