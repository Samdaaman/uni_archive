...
Sensor irProx = Sensor(45, IR_PROX);
...
void resetToReverseThenScan() {Movement::together(MEDIUM, REVERSE); delay(1000); Movement::spin(MEDIUM, RIGHT); delay(1000); resetToState(S_SCANNING, false);}
Watchdog WD_bouncingTooLong = Watchdog(3000, resetToScaning);
Watchdog WD_scanningTooLong = Watchdog(5000, resetToBouncingAround);
Watchdog WD_forwardTooLong = Watchdog(3000, resetToReverseThenScan);
