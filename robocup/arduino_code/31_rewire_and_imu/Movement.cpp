...
const int IO_PIN = 49;
const int DEFAULT_SPEED_SLOW = 300;
const int DEFAULT_SPEED_MEDIUM = 400;
...
void Movement::initialise(bool swapMotors, bool swapLeftPolarity, bool swapRightPolarity, int speedSlow, int speedMedium, int speedFast)
{
    ...
    s_firstServo.attach(3);
    s_secondServo.attach(2);
...
void Movement::veryLargeTurnFast(Side side)
{
    if (side == RIGHT) {
        moveMotors(MEDIUM, FORWARD, MEDIUM, REVERSE);
    } else {
        moveMotors(MEDIUM, REVERSE, MEDIUM, FORWARD);
    }
}
