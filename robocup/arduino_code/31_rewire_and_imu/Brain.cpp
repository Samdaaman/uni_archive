...
static bool foundWallCloseOnSide(Side side) {
    ...
    // }
    // return false;
    return Vision.found(TALL, side, 200 /* TODO: Tune wall max distance */);
}
...
static void routineAvoidOnSide(Side side) {
    ...
    if (lastCalled != 0 && lastCalled + 2000 < millis()) {
        side = lastSide;
    }
    lastSide = side;
    lastCalled = millis();
...
static void actionAvoidOnLean() {
    ...
    Brain.stateChange(&allStates.avoidingFront, C_RESET_FROM_PREVIOUS);
}
