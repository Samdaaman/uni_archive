...
void routineAvoidOnSide(Side side) {
    ...
    Brain.sensorsTick();
    while (topLeftReading.foundObject || topRightReading.foundObject || bottomLeftReading.foundObject || bottomRightReading.foundObject) {
        delay(10);
        Brain.sensorsTick();
    }
    Movement::spin(SLOW, (Side)-side);
...
void BrainClass::InitialiseStates() {
    ...
    allStates.forwardsMindlessly.addWatchdog(&allStates.scanningRandom, 2000);
