...
const int CLAW_OPEN_TIME = 1;
const int CLAW_CLOSE_TIME = 200;
const int ELBOW_SPEED = 1;
const int ELBOW_INITIAL = 1500;
const int ELBOW_UP = 1900; //1700;  // 500 to 2500 which corresponds to degrees
const int ELBOW_DOWN = 580;  // 500 to 2500 which corresponds to degrees
...
void ArmClass::clawDelayAndStopOnError(long milliseconds) {
    long start = millis();
    while (start + milliseconds + 500 /* buffer to be safe */ > millis()) {
        delay(1);
...
{
    static int lastElbowMicroseconds = ELBOW_UP;
    if (!isInitialised) {
        return;
    }
    ...
    if (lastElbowMicroseconds != nextMicroseconds) {
        if (lastElbowMicroseconds > nextMicroseconds) {
            ...
            // Going up
            elbowServo.writeMicroseconds(nextMicroseconds);
        }
