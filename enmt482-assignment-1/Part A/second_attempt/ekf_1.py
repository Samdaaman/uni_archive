...
def main():
    ...
    P[0] = np.zeros((2,2))  # assume the initial belief is accurate
    F = np.array([[1, dt], [0, 1]])  # state transision matrix (df/dx)
    Q = np.array([[1, 0], [0, 0.01]]) # process noise variance (Var[W]) TODO tune
    ...
    for k in range(1,N):
        ...
        # When comparing to the notes H_k=C and R_k=SUM_v frin lecture 11

        # -------------------------------------------------------------
        # Predict
        # -------------------------------------------------------------
        x_k_neg = F @ x[k-1]
        P_k_neg = F @ P[k-1] @ F.T + Q
