...
#
#  Module Description:
#    A collection of sensor model classes.
#    Models must be fitted first
#
...
class SensorModel(ABC):
    ...
    def h(self, x_array: np.ndarray) -> np.ndarray:
        return np.array([self.h_one(x) for x in x_array])

    @abstractmethod
    def h_inv_one(self, z: np.ndarray, guess: Optional[float] = None) -> float:
        """
...
class IRThreePartModel(SensorModel):
    ...
    def h_inv(self, z_array: np.ndarray, initial_guess: float):
        """
        Uses an iterative solver to find three guesses (one for each segment of the piecewise model)
        Evaluates them and attempts to discard guesses that are outside of the segments bounds
        Returns the guess closest to the "initial_guess" parameter
        """
...
def closest_to(target: float, guesses: np.ndarray):
    """
    Helper function to return the closest guess to a target value
    """
