...
#define ENCODER_COUNTS 446 // TODO tune or measure this
static volatile int16_t quad_postition = 0;

#define PHASE_A GPIO_PIN_3
#define PHASE_B GPIO_PIN_2

void quad_interrupt(void)
{
    static bool a_dash = false;
    static bool b_dash = false;
    const uint32_t gpio = GPIOPinRead(GPIO_PORTA_BASE, PHASE_A | PHASE_B);
    ...
    GPIOIntClear(GPIO_PORTA_BASE, PHASE_A | PHASE_B);
}
...
void quad_init(void)
{
    ...
    GPIOPinTypeGPIOInput(GPIO_PORTA_BASE, pins);
    ...
    GPIOIntDisable(GPIO_PORTA_BASE, pins);
    GPIOIntRegister(GPIO_PORTA_BASE, quad_interrupt);

    GPIOIntTypeSet(GPIO_PORTA_BASE, pins, GPIO_BOTH_EDGES);
    GPIOIntEnable(GPIO_PORTA_BASE, pins);
}
...
void adc_init(void)
{
    ...
    ADCSequenceStepConfigure(ADC0_BASE, 3, 0, ADC_CTL_CH2 | ADC_CTL_IE | ADC_CTL_END); // use AIN1
...
void greet(void* args)
{
    ...
    while (true) {
        ...
        usnprintf(greeting, sizeof(greeting), "Hello %i,  quad=%i  deg=%i  height=%i%%  duty=%i\r\n", count++, quad_postition, deg, height, current_duty);
        uart_puts(greeting);
...
void duty(void* args)
{
    ...
    while (true) {
        ...
        pwm_set_main_rotor_dutycycle(current_duty);
        pwm_set_tail_rotor_dutycycle(100 - current_duty);
...
int main(void)
{
    ...
    toggle_main_PWM_output(1);
    toggle_tail_PWM_output(1);

    xTaskCreate(&greet, "greet", 256, NULL, 0, NULL);
    xTaskCreate(&read, "read", 256, NULL, 0, NULL);
    xTaskCreate(&duty, "duty", 256, NULL, 0, NULL);
