...
void greet(void* args) {
    ...
    while (true) {
        ...
        uint32_t height = (2300 - pui32ADC0Value[0]) / 5;
        height = height < 0 ? 0 : height > 100 ? 100 : height;

        char greeting[100];

        usnprintf(greeting, sizeof(greeting), "Hello %i %i  (Height=%i%%)!\r\n", count++, pui32ADC0Value[0], height);
        uart_puts(greeting);
...
void adc_init(void)
{
    ...
    //
    uart_puts("ADC ->\n");
    uart_puts("  Type: Single Ended\n");
    uart_puts("  Samples: One\n");
    uart_puts("  Update Rate: 250ms\n");
    uart_puts("  Input Pin: AIN0/PE3\n");
    uart_puts("  Analog source: Orbit potentiometer\n\n");
